...
const deleteNotification = async (req, res, next) => {
  try {
    const { commentId, type, senderUser, userId, id } = req.body

    // const deleteNotification = await NotificationService.getNotificationById(id)
    ...
    if (type === 'like') {
      ...
      if (!senderUser || !senderUser.match(/^[0-9a-fA-F]{24}$/)) {
        const error = new HttpError('You must give a valid id', 401)
        return next(error)
      }
      const result = await NotificationService.deleteNotificationComment(
        senderUser,
        commentId
      )
      if (!result) {
        const error = new HttpError(`Can't delete the notification `, 400)
        return next(error)
      }
      const notifications = await NotificationService.getNotification(userId)
      main.io.emit('delete-notification', notifications)
      res
        .status(200)
...
const markOneNotificationasread = async (req, res, next) => {
  try {
    ...
    const notifications = await NotificationService.getNotification(userId)
    main.io.emit('update-notification', notifications)
